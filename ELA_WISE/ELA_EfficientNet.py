...
def ELA_WISE(args):
    ...
    # 分割數據為訓練集和測試集
    X_train, X_test, y_train, y_test = train_test_split(resized_ALL_images, ALL_labels, test_size=0.2, random_state=42)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if auc_train > best_auc:
            best_auc = auc_train
            torch.save(model.state_dict(), f'ELA_WISE_model_{int(round(best_auc, 3)*1000)}.pth')
