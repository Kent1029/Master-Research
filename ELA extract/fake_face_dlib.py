...
def crop_region_face(img, landmarks,crop_output_path,counter):
    """
    裁剪出landmarks指定的区域。
    :param img: 图像。
    ...
    crop_image_filename = f'fake_face_image{counter}.png'
...
def ELA(png_image_path,ela_output_path,counter):
    #  載入PNG圖像
    ...
    #ela_image.show()
    # 保存ELA圖像
    ela_image_filename = f'fake_ela_image{counter}.jpg'
...
def get_region_face_landmark_mask(ela_image_path,mask_output_path,counter):
    # 載入ELA圖像
    ...
    mask_image_filename = f'fake_mask_image{counter}.png'
